...
- (void)fadeTo:(float)volume duration:(NSTimeInterval)time
{
...
{
    [sound setVolume:0.0f];
    [self fadeTo:1.0f duration:time];
}

- (void)fadeOut:(NSTimeInterval)time
{
    [self fadeTo:0.0f duration:time];
}
...
- (void)prepareToPlayWithSound:(id)soundOrName
{
    ...
    [sound prepareToPlay];
    self.currentMusicLength = sound.duration;
}
